...
sequenceDiagram
    ...
    %% Event Deactivation
    Organizer->>Frontend: Deactivate Event (Event ID)
    Frontend->>Soroban: deactivate_event(eventId)
    Soroban-->>Frontend: Success
    Frontend-->>Organizer: Shows "Event deactivated successfully"
```
...
### 5. Event Deactivation

1. **Action**: Once the event has concluded, the organizer can deactivate the event via the dashboard.
2. **Execution**: A `deactivate_event` transaction is signed and submitted to Soroban.
3. **Enforcement**: Once the event is deactivated on-chain, the Soroban contract strictly rejects any further `issue_ticket` or `check_in` transactions for that event, ensuring the finality of the event's lifecycle.
