...
- **Key Flow**: It orchestrates the process of saving metadata off-chain *before* deploying the authoritative ID on-chain.
...
- **Ticket Issuance**: The contract checks `organizer.require_auth()` and verifies the caller matches the stored `organizer` for the event.
- **Check-In**: The operator performing the check-in must match the `organizer` address exactly, preventing rogue actors from validating tickets.
