import { TransactionBuilder, rpc, Contract, xdr, Transaction, Account } from '@stellar/stellar-sdk';
...
export async function submitContractTx(
  ...
  if (rpc.Api.isSimulationError(simulated)) {
    ...
    if (method === 'check_in') {
      if (simulationError.includes('Error(Contract, #3)')) {
        throw new Error('You are not authorized to perform this check-in.');
      }
      if (simulationError.includes('Error(Contract, #7)')) {
        throw new Error('Event is inactive. This ticket cannot be checked in.');
      }
    }

    if (method === 'issue_ticket') {
      if (simulationError.includes('Error(Contract, #7)')) {
        throw new Error('Event is inactive. No new tickets can be issued for this event.');
      }
    }
  ...
  try {
    ...
    signedTxXdr =
      typeof result === 'string'
        ? result
        : (result as any).signedTxXdr || (result as any).signedTransaction;
    ...
    throw new Error('Wallet signing failed: ' + (err?.message || JSON.stringify(err)));
  }
  ...
  if (response.status === 'ERROR') {
    ...
    const rawError = (response as any).errorResultXdr || JSON.stringify(response.errorResult);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    throw new Error(`Transaction failed on-chain: ${(result as any).resultXdr || result.status}`);
  }

  throw new Error('Transaction timed out waiting for confirmation.');
}

export async function simulateContractRead(method: string, args: xdr.ScVal[]): Promise<xdr.ScVal> {
  const contract = new Contract(STELLAR_CONTRACT_ID);
