...
export default function Verify() {
      ...
      try {
        ...
        if (urlMatch) {
          ...
          if (!ticketMatch || !eventMatch) {
            throw new Error(
              "Invalid ticket payload. Expected JSON, URL, or plain text."
            );
          }
  ...
  const handleCheckIn = async () => {
    ...
    try {
       const hash = await checkIn(pubKey, BigInt(verificationResult.ticketId), (s) => setStatus(`Blockchain status: ${s}`));
       setTxHash(hash);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200">
        ...
        <p className="text-gray-500 mb-6">Scan an attendee&apos;s QR code or paste the payload here to verify on-chain.</p>
        ...
        <form onSubmit={handleVerify} className="space-y-4">
          <textarea 
            ...
            rows={4} 
            className="w-full border-gray-300 rounded-md shadow-sm p-2 border focus:ring-primary focus:border-primary font-mono text-sm" placeholder='{"ticketId": "123", "eventId": "456"}' />
          <button type="submit" className="w-full py-3 bg-gray-900 text-white rounded-md font-medium hover:bg-gray-800 transition-colors">
            Verify on Blockchain
          </button>
        </form>

        {status && <div className="mt-4 p-3 bg-blue-50 text-blue-700 rounded-md text-sm text-center font-medium">{status}</div>}
        {txHash && <div className="mt-2 p-3 bg-green-50 text-green-700 rounded-md text-sm text-center break-all font-mono">Tx Hash: {txHash}</div>}
      </div>

      {verificationResult && (
        <div className={`p-8 rounded-xl shadow-sm border ${verificationResult.valid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          <div className="flex items-center justify-center mb-6">
            <span className="text-4xl mr-3">{verificationResult.valid ? '✅' : '❌'}</span>
            <h3 className={`text-2xl font-bold ${verificationResult.valid ? 'text-green-800' : 'text-red-800'}`}>
          ...
          {verificationResult.valid ? (
            <div className="bg-white p-4 rounded-lg border border-green-100">
              <p><strong>Ticket ID:</strong> {verificationResult.ticketId}</p>
              <p><strong>Event ID:</strong> {verificationResult.eventId}</p>
              <p className="mt-2">
                <strong>Status:</strong> {verificationResult.checkedIn ? (
          ...
          {verificationResult.valid && !verificationResult.checkedIn && (
            <div className="mt-8">
              <p className="text-sm text-green-700 mb-3 text-center">Only authorized operators can check in attendees.</p>
              {!pubKey ? (
                <button onClick={connect} className="w-full py-3 bg-green-600 text-white rounded-md font-medium hover:bg-green-700 transition-colors">
                  Connect Wallet to Check-In
                </button>
              ) : (
                <button onClick={handleCheckIn} className="w-full py-3 bg-green-600 text-white rounded-md font-medium hover:bg-green-700 transition-colors">
