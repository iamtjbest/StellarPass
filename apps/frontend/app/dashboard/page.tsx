...
import { createEvent, issueTicket } from "../../lib/stellar/contract";
...
export default function Dashboard() {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const hash = await createEvent(
        ...
        generatedEventId,
        metadataRef,
        (s) => setStatus(`Blockchain status: ${s}`)
      );
      ...
      setStatus(
        `Event created successfully. Event ID: ${generatedEventId.toString()}`
      );
  ...
  const ticketUrl = issuedTicketId
    ? `${window.location.origin}/ticket/${issuedTicketId}`
    : "";
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          >
            ...
            >
              Deploy Event to Soroban
            </button>

            {status && (
              <div className="p-3 rounded-md text-sm text-center bg-blue-50 text-blue-700">
                {status}
              </div>
            )}
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={!pubKey || !eventId || !recipient}
              className="w-full py-3 bg-primary text-white rounded-md font-medium hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Issue Ticket
            </button>
